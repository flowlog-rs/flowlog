...
compiler: the plan graph written at compile time, and the reader that joins
a profiled run's metrics back onto it. Consumed by
...
- `plan` — `PlanGraph`: nodes, rules, blocks, and the builder that
  records them at compile time.
- `metrics` — `metrics::read()` joins a run's logs onto a plan, yielding
  per-transaction `Snapshot`s.
