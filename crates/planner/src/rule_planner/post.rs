...
impl RulePlanner {
    ...
    ///   (supports variables, arithmetic, and aggregation in the head).
    ...
    /// Update the last transformation's output layout in-place for any head
    /// (variables, arithmetic expressions, or aggregation).
    fn update_last_transformation_layout(&mut self, catalog: &Catalog, head_args: &[HeadArg]) {
        ...
        let name_to_sig = Self::build_name_to_output_signatures(catalog, last_tx);
    ...
    fn create_post_transformation(&mut self, catalog: &Catalog, head_args: &[HeadArg]) {
        ...
        let (input_fake_sig, input_kv_layout) =
            if let Some(last_tx) = self.transformation_infos.last() {
                (last_tx.output_info_fp(), last_tx.output_kv_layout().clone())
            } else {
                // No previous transformations; use the first positive atom's fingerprint and layout
                (
    ...
    ///
    /// This respects any reordering performed by previous transformations.
    fn build_name_to_output_signatures(
        catalog: &Catalog,
