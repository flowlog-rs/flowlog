...
impl Constraints {
    ...
    /// content-based arrangement sharing (see
    /// [`TransformationFlow::content_key`]). Equality constraints are
    /// conjunctive — `x = 5, y = z` keeps exactly the same rows as
    /// `y = z, x = 5` — so each kind is hashed as an order-independent multiset.
    /// The two kinds are combined under a tag so a constant-equality can never
    /// collide with a variable-equality of the same shape.
    ///
    /// [`TransformationFlow::content_key`]: super::TransformationFlow::content_key
...
mod content_key_tests {
    ...
    /// filters in a different order would fail to share their arrangement).
    #[test]
    fn content_key_ignores_order() {
        ...
        let c2 = Constraints::new(
            // same constraints, constants reversed
    ...
    fn content_key_distinguishes_real_differences() {
        ...
        // Different constrained column.
        assert_ne!(
            base.content_key(),
            Constraints::new(vec![(arg(1), ConstType::Int(5))], vec![]).content_key(),
            "constrained column must matter"
        );
        // A constant-equality and a variable-equality must not collide.
        assert_ne!(
