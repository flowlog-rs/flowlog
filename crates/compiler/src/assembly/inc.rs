...
use super::AssemblyParts;

/// Emit the complete incremental-mode `fn main() { ... }` token stream.
pub(crate) fn gen_incremental_main(p: &AssemblyParts, compiler: &crate::Compiler) -> TokenStream {
    let AssemblyParts {
        edb_inputs,
        handle_binding,
        dataflow_return,
    ...
    let time_profile_write = compiler.gen_time_profile_write_incremental();
    let memory_profile_write = compiler.gen_memory_profile_write_incremental();
