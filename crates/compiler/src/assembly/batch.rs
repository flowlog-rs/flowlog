...
pub(crate) fn gen_batch_main(p: &AssemblyParts, compiler: &crate::Compiler) -> TokenStream {
    let AssemblyParts {
        edb_inputs,
        handle_binding,
        dataflow_return,
    ...
    let time_profile_write = compiler.gen_time_profile_write_batch();
    let memory_profile_write = compiler.gen_memory_profile_write_batch();
