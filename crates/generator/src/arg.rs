...
fn numeric_arithmetic_op_tokens(op: &ArithmeticOperator) -> TokenStream {
    match op {
        ...
        _ => {
            unreachable!(
                "Generator error: string operator {} found in numeric arithmetic expression",
                op
            )
        }
    }
    ...
    fn build_row_args_arithmetic_expr(
        ...
        self.build_arithmetic_expr(expr, string_intern, &|arg| match arg {
            TransformationArgument::KV((_, idx)) => {
                let ident = fields
                    .get(*idx)
                    .expect("Generator error: row index out of bounds in row builder");
            ...
            _ => unreachable!("Generator error: unexpected argument type in row builder"),
        })
    }
