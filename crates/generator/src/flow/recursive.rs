...
impl Generator {
    ...
    fn collect_unions(
        ...
        for (idb_fp, head_fps) in idb_to_heads_map {
            ...
            let (head, tail) = sources
                .split_first()
                .expect("Generator error: at least one source collection for union");
        ...
        let leave_exprs: Vec<TokenStream> = leave_fps
            ...
            .map(|(fp, target)| {
                let next_ident = next.get(fp).expect(
                    "Generator error: leave relation missing from next bindings during recursion",
                );
    ...
    fn gen_feedback_stmts(
        ...
        for (fp, recursive_ident) in recursive_bindings {
            let next_ident = next_bindings.get(fp).unwrap_or_else(|| {
                panic!("Generator: recursive relation fp {fp} missing next binding")
            });
    ...
    fn build_until_conditions(
        ...
        let first_next = next_bindings.get(&first.fp).unwrap_or_else(|| {
            panic!(
                "Generator: until relation fp {} missing from next_bindings",
                first.fp
            )
        });
        ...
        for (conn, rel) in until_group.rest() {
            let fp = rel.fp;
            let next_ident = next_bindings.get(&fp).unwrap_or_else(|| {
                panic!("Generator: until relation fp {fp} missing from next_bindings")
            });
...
///
/// Built by `Generator::prepare_loop_condition`; consumed by `build_feedback_expr`.
