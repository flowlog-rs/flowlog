...
impl Generator {
        ...
        self.global_fp_to_type
            ...
            .and_modify(|existing| {
                ...
                assert!(
                    ...
                    "Generator error: type mismatch for fingerprint 0x{:016x}",
                    output_fingerprint
                );
            })
            .or_insert_with(|| {
                let keys = key_types
                    .into_iter()
                    .map(|t| t.expect("Generator error: cannot infer type for all-constant position for intermediate relation without IDB declaration"))
                    .collect();
    ...
    fn assert_join_key_type_compat(
        ...
        if let (Some(rf), Some(rt)) = (right_fp, right_type) {
            assert!(
                left_type.0 == rt.0,
                "Generator error: key type mismatch for fingerprints 0x{:016x} and 0x{:016x}",
                left_fp,
                rf
            );
        }
        ...
        match factor {
            ...
            FactorArgument::Var(TransformationArgument::Jn((is_left, is_key, idx))) => {
                let base = if *is_left {
                    ...
                    right_type
                        .expect("Generator error: right input type missing for join transformation")
                };
            ...
            FactorArgument::FnCall { name, args } => {
                let ext = self
                    ...
                    .iter()
                    .find(|e| e.name() == name)
                    .unwrap_or_else(|| panic!("Generator error: UDF '{}' not declared", name));

                // 1. Assert argument count.
                assert_eq!(
                    args.len(),
                    ext.params().len(),
                    "Generator error: UDF '{}' expects {} args but got {}",
                    name,
                    ext.params().len(),
                    args.len(),
                );
                ...
                for (i, (arg, param)) in args.iter().zip(ext.params()).enumerate() {
                    ...
                    match arg_type {
                        Some(at) => {
                            ...
                            at,
                            *param.data_type(),
                            "Generator error: UDF '{}' param {} ('{}') expects {:?} but got {:?}",
                        ...
                        None => {
                            ...
                            assert!(
                                ...
                                "Generator error: UDF '{}' param {} ('{}') expects {:?} but got a numeric literal",
    ...
    pub(crate) fn infer_expr_type(
        ...
        for (op, factor) in &expr.rest {
            if let Some(dt) = self.infer_factor_type(factor, left_type, right_type) {
                match inferred {
                    ...
                    Some(existing) => assert!(
                        existing == dt,
                        "Generator error: mixed data types in arithmetic expression: {:?} vs {:?}",
                        existing,
                        dt
                    ),
                }
            ...
            if let Some(dt) = inferred {
                if dt == DataType::Bool {
                    panic!("Generator error: arithmetic operations are not supported on Bool type");
                } else if dt == DataType::String {
                    assert!(
                        matches!(op, ArithmeticOperator::Cat),
                        "Generator error: arithmetic operator {:?} is not allowed on string type, use 'cat'",
                        op
                    );
                } else {
                    assert!(
                        !matches!(op, ArithmeticOperator::Cat),
                        "Generator error: 'cat' operator is not allowed on numeric type {:?}",
                        dt
                    );
                }
            }
    ...
    pub(crate) fn find_global_data_type(
        ...
        self.global_fp_to_type.get(&fingerprint).unwrap_or_else(|| {
            panic!(
                "Generator error: input type missing for fingerprint 0x{:016x}",
                fingerprint
            )
        })
    }
