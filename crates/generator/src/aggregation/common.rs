...
fn aggregation_result_expr(op: &AggregationOperator, agg_type: DataType) -> TokenStream {
    match op {
        AggregationOperator::Count => match agg_type {
            ...
            _ => {
                panic!("Generator error: count aggregation result should be numeric type")
            }
        },
        AggregationOperator::Sum => match agg_type {
            ...
            _ => panic!("Generator error: sum aggregation result should be numeric type"),
        },
        AggregationOperator::Min => match agg_type {
            ...
            _ => {
                panic!("Generator error: min aggregation is not supported on non-numeric type")
            }
        },
        AggregationOperator::Max => match agg_type {
            ...
            _ => {
                panic!("Generator error: max aggregation is not supported on non-numeric type")
            }
        },
        AggregationOperator::Avg => match agg_type {
            ...
            _ => panic!("Generator error: avg aggregation result should be numeric type"),
        },
    }
}
