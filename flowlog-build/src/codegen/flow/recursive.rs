    ...
    fn build_leave_outputs(
        ...
        let leave_exprs: Vec<TokenStream> = leave_fps
            ...
            .map(|fp| -> Result<TokenStream, CodegenError> {
                    ...
                    && self.config.is_datalog_batch()
                {
                    ...
                    let op_name = format!("ReduceLift: {}", self.display_name(*fp));
                    return Ok(quote! {
                        ::flowlog_runtime::operators::flowlog_reduce_lift(
                            #next_ident, #op_name, #kind, #split,
                        )
                            .leave(scope)
                    });
                }
        ...
        for (fp, target) in leave_fps.iter().zip(targets.iter()) {
                ...
                && self.config.is_datalog_batch()
            {
                ...
                let finish_name = format!("ReduceFinish: {}", self.display_name(*fp));
                post_leave_stmts.push(quote! {
                    let #target =
                        ::flowlog_runtime::operators::flowlog_reduce_finish(#target, #finish_name, #merge);
                });
            }
        }
        ...
        for (conn, rel) in until_group.rest() {
            ...
            match conn {
                LoopConnective::And => stmts.push(quote! {
                    let #combined = ::flowlog_runtime::operators::flowlog_join(
