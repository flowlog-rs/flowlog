...
//! as two closures, plus a token naming the aggregation. Which semiring
...
/// Runtime token naming the aggregation.
pub(super) fn aggregation_token(op: AggregationOperator) -> TokenStream {
...
mod tests {
    ...
    /// Every operator is a bare token; the reported type rides on the merge
    ...
    fn token_names_the_aggregation(#[case] op: AggregationOperator, #[case] expected: &str) {
        assert_eq!(
            normalized(aggregation_token(op)),
