...
impl DataType {
    ...
    pub fn any_leaf(&self, pred: &impl Fn(&DataType) -> bool) -> bool {
        match self {
...
impl TypeRegistry {
    ...
    #[must_use]
    #[inline]
    pub fn root_primitive(&self, id: TypeId) -> DataType {
    ...
    #[must_use]
    pub fn primitive_id(&self, dt: DataType) -> TypeId {
        let idx = PRIM_NAMES
            .iter()
            .position(|(p, _)| p == &dt)
    ...
    #[must_use]
    pub fn is_widening(&self, sub: TypeId, sup: TypeId) -> bool {
        let mut cur = sub;
        loop {
            if cur == sup {
                return true;
    ...
    #[must_use]
    pub fn meet(&self, a: TypeId, b: TypeId) -> Option<TypeId> {
