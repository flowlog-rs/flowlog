...
pub enum ParseError {
    ...
    /// `.input R(delimiter="...")` that names no single byte to split cells
    /// on, or names one the line reader has already consumed.
    #[error("`.input` delimiter must be one ASCII character, not \"{}\"", .value.escape_debug())]
    InvalidDelimiter { span: Span, value: String },

    /// `.output R(IO="...")` naming a sink FlowLog does not write. Only a
    /// delimited text file exists, so anything else would be written as a
    ...
    /// follow Rust syntax (quoted with Rust's escape alphabet, or raw), so
    /// an unknown escape is an error and a pattern that needs a literal
    /// backslash belongs in a raw string.
    ...
    fn to_diagnostic(&self) -> CsDiagnostic<FileId> {
        ...
        match self {
            ...
            ParseError::UnknownOutputIo { span, .. } => base
                .with_labels(primary_only(*span))
                .with_notes(vec![
                    "FlowLog writes a delimited text file: drop the `IO=` or set it to \
                     \"file\", and use the compiler's output directory to choose where \
                     rows go"
                        .into(),
                ]),
